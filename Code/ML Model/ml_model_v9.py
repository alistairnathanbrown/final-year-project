...
# MODEL WITH BEST HYPER PARAMETERS AND KMEANS 7, UPDATED TO RDS5, WITH FEATURE INTERACTIONS
...
data["Wind Speed × Capacity"] = data["Annual Wind Speed (m/s)"] * data["Capacity (MW)"]
data["Inflation × Project Cost"] = data["Inflation in Project Country (HCPI)"] * data["Estimated Project Cost (GBP)"]
data["Debt-to-GDP × Credit Rating"] = data["Government Debt as Percentage of GDP"] * data["Country Credit Rating"]

# Impute missing values in numerical columns with the median
num_imputer = SimpleImputer(strategy="median")
data[numerical_cols + ["Wind Speed × Capacity", "Inflation × Project Cost", "Debt-to-GDP × Credit Rating"]] = num_imputer.fit_transform(data[numerical_cols + ["Wind Speed × Capacity", "Inflation × Project Cost", "Debt-to-GDP × Credit Rating"]])
...
# Drop 'Longitude' and 'Latitude' columns and 'Country'
...
categorical_cols = ["Installation Type", "Location Cluster"]
numerical_cols = [
...
numerical_cols += ["Wind Speed × Capacity", "Inflation × Project Cost", "Debt-to-GDP × Credit Rating"]
